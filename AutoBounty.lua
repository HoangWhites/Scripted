...
function GetNpcCount(mobname)
    local number1
    local count = 0
    for k, v in pairs(game.Workspace.Enemies:GetChildren()) do
        if v.Name == mobname then
            if not number1 then
                number1 = v.HumanoidRootPart.Position
            else
                number1 = number1 + v.HumanoidRootPart.Position
            end
            count = count + 1
        end
    end
...
function BringMob(value)
        local Enemies = game:GetService("Workspace").Enemies:GetChildren()
        if #Enemies > 0 then
            for nigga = 1, #Enemies do
                local TableMobSmart = {}
                for r, v in next, game.Workspace.Enemies:GetChildren() do
                    if v.Name == value and not TableMobSmart[v.Name] then
                        TableMobSmart[v.Name] = GetNpcCount(v.Name)
                    end
                end
                for r, v in next, game.Workspace.Enemies:GetChildren() do 
                    for r2, v2 in pairs(v:GetChildren()) do
                    ...
                    for r2, v2 in next, TableMobSmart do
                        if r2 and v.Name == r2 then
                            tablepos = {
                                [1] = v2.X,
                                [2] = v2.Y,
                                [3] = v2.Z
                            }
                            if (v.HumanoidRootPart.Position - CFrame.new(tablepos[1], tablepos[2], tablepos[3]).Position).Magnitude > 3 and (plr.Character.HumanoidRootPart.Position - CFrame.new(tablepos[1], tablepos[2], tablepos[3]).Position).Magnitude < 349 then
                                v.HumanoidRootPart.CFrame = CFrame.new(tablepos[1], tablepos[2], tablepos[3])
