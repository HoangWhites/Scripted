...
function ToTarget(TargetCFrame)
    ...
    if plr.Character:FindFirstChild("Humanoid") and plr.Character.Humanoid.Health > 0 and plr.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if Aaa <= 200 then
            ...
            if config["Bypass Teleport"] and (plr.Character.HumanoidRootPart.Position - TargetCFrame.Position).Magnitude >= 3875 and plr.Character:WaitForChild("Humanoid").Health > 0 then
                if plr.Character.Humanoid.Health > 0 then
                    wait(.4)
                    if tween then tween:Pause() tween:Cancel() end
...
Attack = function()
    ...
    if ac and ac.equipped then
        ...
        if #bladehit > 0 then
            ...
            local mmmb = ac.anims.basic[3]
            if plr.Character:FindFirstChildOfClass('Tool') and ac.blades and ac.blades[1] then 
                Rig.FireServer(Rig, "hit", bladehit, mmmb and 3 or 2, "")
            end
        end
    end
end
...
function DetectEnemyPart()
    local TableEnemyPart = {}
            ...
            for r1, v1 in next, v do
                for r2, plr in next, v1.Task do
                    if plr > 1 then
                        ...
                        table.insert(TableEnemyPart, r2)
                    end
                end
            end
        end
    ...
    return TableEnemyPart
end
local enemypart = DetectEnemyPart()
ReloadFolderMob = function()
    for r, v in pairs(game.Workspace.FolderMob:GetChildren()) do
        v:Destroy()
    end
    for r, v in pairs(enemypart) do
        if v then
            if v:IsA("Model") and v:FindFirstChild("HumanoidRootPart") then
                PartNew = Instance.new("Part")
                PartNew.CFrame = v.HumanoidRootPart.CFrame
                PartNew.Name = v.Name
                PartNew.Anchored = true
                PartNew.Transparency = 1
                PartNew.CanCollide = false
                PartNew.Parent = game.Workspace.FolderMob
...
ReloadFolderMob()
GetEnemyPartList = function(Enemy)
    EnemyName = RemoveLevelTitle(Enemy)
    EnemyReturnter = {}
    for i, v in pairs(game.Workspace.FolderMob:GetChildren()) do
        if v.Name == EnemyName then
            table.insert(EnemyReturnter, v)
        end
    end
    return EnemyReturnter
end
GetEnemySpawnPart = function(MobName)
    local TablePart = {}
    for r, v in pairs(game.Workspace.FolderMob:GetChildren()) do
        if not table.find(TablePart, v.Name) then
            table.insert(TablePart, v.Name)
        end
    end
    if string.find(WS["_WorldOrigin"].EnemySpawns:GetChildren()[1].Name, "Lv.") then
        for r, v in pairs(TablePart) do
            local b = v
            v = tostring(v:gsub(" %pLv. %d+%p", ""))
            if ((typeof(MobName) == "table" and table.find(MobName, v.Name)) or (typeof(MobName) == "string" and (string.find(v.Name, MobName) or v.Name == MobName))) then
                return b
            end
        end
    else
        if typeof(MobName) == "table" then
            for r, v in pairs(MobName) do
                return v
            end
        elseif typeof(MobName) == "string" then
            return MobName
        end
    end
end
...
CheckQuest = function()
    ...
    local phuongngu = 0
    if Levels >= 700 and yamate(1) then
        Mob = "Galley Captain"
        NameQuest = "FountainQuest"
        ID = 2
    elseif Levels >= 1500 and yamate(2) then
        Mob = "Water Fighter"
...
MobDataQuest = function()
    local mobcontainer
    if GetQuestContainer() then
...
GetDoubleQuest = function()
    local returner = {}
    CheckQuest()
    if config["Double Quest"] and GetQuestContainer() and MobDataQuest() == Mob  and #GetMobDoubleQuest() >= 2 and plr.Data.Level.Value >= 10 then
        for _, v in next, Quests do
            for r, i in next, v do
                for d, b in next, i.Task do
...
ShopTab:AddDropdown({
    ...
    Options = {"Black Legs", "Electro", "Fishman Karate", "Dragon Claw", "Superhuman", "Death Step", "Electric Claw", "Sharkman Karate", "Dragon Talon", "Godhuman", "Sanguine Art"},
    Default = "",
...
ToggleFarm:AddToggle({
    Name = "Summon Cake Prince",
    Default = config["Summon Cake Prince"],
    Callback = function(Value)
        Save("Summon Cake Prince", Value)
    end
})
        ...
        if CheckingStack("Normal Farm") then
            ...
            if config["Method Farm"] == "Levels Farm" then
                if plr.Data.Level.Value >= 1 and plr.Data.Level.Value <= 119 then
                    if DetectEnemy({"Shanda", "Royal Squad"}, true) then
                        local v = DetectEnemy({"Shanda", "Royal Squad"}, true)
                        KillMob(v, true, function() return not config["Auto Farm"] or CheckingStack("Normal Farm") end) 
                    else
                        for r, v in next, game.Workspace.FolderMob:GetChildren() do
                            if v:IsA("Part") and table.find({"Shanda", "Royal Squad"}, v.Name) then
                                repeat wait()
                                    Velocity = true
                                    ToTarget(v.CFrame * CFrame.new(0, 40, 0))
                                until DetectEnemy({"Shanda", "Royal Squad"}) or (v.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 or config["Auto Farm"] == false
                            end 
                        end
                    end
                else
                    if plr.PlayerGui.Main.Quest.Visible then
                        if DetectEnemy(MobDataQuest()) then
                            for r, v in next, game.Workspace.Enemies:GetChildren() do
                                if v.Name == MobDataQuest() then
                            ...
                            for r, v in next, GetEnemyPartList(GetEnemySpawnPart(MobDataQuest())):GetChildren() do
                                if v:IsA("Part") and string.find(MobDataQuest(), v.Name) or v.Name == MobDataQuest() then
                                    repeat wait()
                                        Velocity = true
                                        ToTarget(v.CFrame * CFrame.new(0, 30, 0))
                                    until DetectEnemy(MobDataQuest()) or (v.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 or config["Auto Farm"] == false
                                end 
                            end
                        end
                    else
                        if not plr.PlayerGui.Main.Quest.Visible or CheckStringQuests("Urban") or CheckStringQuests("Deandre") or CheckStringQuests("Diablo") then
                            ...
                            if GetDistance(QuestCFrame.Position) <= 10 then
                                FireRemotes(2, "StartQuest", tostring(GetDoubleQuest()["NameQuest"]), GetDoubleQuest()["ID"])
                            else
                                ToTarget(QuestCFrame)
                            end
                        end
                    end
                end
            elseif config["Method Farm"] == "Cake Prince Farm" then
                if DetectEnemy("Cake Prince", true) then
                    local v = DetectEnemy("Cake Prince", true)
                    if v then
                        if GetDistance(v.HumanoidRootPart.Position) <= 950 then
                            KillMob(v, false, function() return config["Auto Farm"] == false or CheckingStack("Normal Farm") end)
                        else
                    ...
                    if config["Claim Quest"] and not CheckStringQuests("Cookie Crafter") then
                        if GetDistance(CFrame.new(-2021.32007, 37.7982254, -12028.7295).Position) <= 10 then
                            print("Get Quest")
                            FireRemotes(2, "StartQuest", "CakeQuest1", 1)
                        else
                    ...
                    elseif not config["Claim Quest"] or (CheckStringQuests("Cookie Crafter") and config["Claim Quest"]) then
                        if not DetectEnemy(TableCakeMobs) then
                            for r, v in next, GetEnemyPartList(GetEnemySpawnPart(TableCakeMobs)):GetChildren() do
                                if v:IsA("Part") and table.find(TableCakeMobs, v.Name) then
                                    repeat wait()
                                        Velocity = true
                                        ToTarget(v.CFrame * CFrame.new(0, 30, 0))
                                    until DetectEnemy(TableCakeMobs) or (v.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 or config["Auto Farm"] == false
                                end 
                            end
                        else
            ...
            elseif config["Method Farm"] == "Bones Farm" then
                ...
                elseif not config["Claim Quest"] or (CheckStringQuests("Posessed Mummy") and config["Claim Quest"]) then
                    if not DetectEnemy(TableHauntedMobs) then
                        for r, v in next, GetEnemyPartList(GetEnemySpawnPart(TableHauntedMobs)):GetChildren() do
                            if v:IsA("Part") and table.find(TableHauntedMobs, v.Name) then
                                repeat wait()
                                    Velocity = true
                                    ToTarget(v.CFrame * CFrame.new(0, 30, 0))
                                until DetectEnemy(TableHauntedMobs) or (v.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 or config["Auto Farm"] == false
                            end 
                        end
                    else
...
spawn(function()
    while wait() do
        if config["Auto Farm"] then
            if config["Summon Cake Prince"] then
                local CakePrinceSpawner = game.ReplicatedStorage.Remotes.CommF_:InvokeServer("CakePrinceSpawner", true)
                if CakePrinceSpawner:find("open the portal now") then
                    spawn(function()
