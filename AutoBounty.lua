    ...
    while wait() do
        pcall(function()
            for r, v in next, game.Workspace.Enemies:GetChildren() do
                if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and plr:DistanceFromCharacter(v.HumanoidRootPart.Position) < 70 then
                    local ac = CombatFr1.activeController
                    if ac and ac.equipped and not plr.Character.Stun.Value ~= 0 then 
...
function InMyNetwork(object)
    if isnetworkowner then
        ...
        return plr.Character:FindFirstChild("HumanoidRootPart") and GetDistance(object.Position, plr.Character:FindFirstChild("HumanoidRootPart").Position) <= Scibidi["Bring Range"]
    end
end
...
function GetMidBring(MobName, b2)
    ...
    for _, enemy in pairs(game.Workspace.Enemies:GetChildren()) do
        if enemy.Name == MobName then
            ...
            if hrp and humanoid and humanoid.Health > 0 and GetDistance(hrp.Position, b2.Position) <= Scibidi["Bring Range"] then
...
function BringMob(enable)
    xpcall(function()
        if not enable or not plr.Character:FindFirstChild("HumanoidRootPart") then 
            return 
        end
...
function TweenMobSpawn(value, loopstop)
    ...
    for r, v in next, game.Workspace.FolderMob:GetChildren() do
        if v:IsA("Part") and ((typeof(value) == "table" and table.find(value, v.Name)) or (typeof(value) == "string" and string.find(v.Name, value))) then
            ...
            until CheckMob(value) or loopstop or getdistance(v.Position) <= 10 or QuaytronQuaytron
        end 
    end
end
