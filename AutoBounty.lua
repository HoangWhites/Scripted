...
function to(TargetCFrame)
    ...
    if (TargetCFrame.Position - plr.Character.HumanoidRootPart.Position).Magnitude >= 3000 and not CheckInCombat() then
        bypass(TargetCFrame)
    end
            ...
            if Hop == true then
                ...
                elseif ((plr.Character.Humanoid.Health > Config["Settings"]["Panic Mode"][2] and plr.Character.Humanoid.Health > Config["Settings"]["Panic Mode"][3] and Config["Settings"]["Panic Mode"][1]) or (not Config["Settings"]["Panic Mode"][1])) then 
                    if (enemy.Character.HumanoidRootPart.Position - plr.Character.HumanoidRootPart.Position).Magnitude >= 150 then
                        ...
                        if enemy:DistanceFromCharacter(plr.Character.Head.Position) < 100 and tick() - Nguvai >= 30 and not CheckInComBat() then
                            FindNewEnemy()
                            Nguvai = tick()
                        end
                        ...
                        to(enemy.Character.HumanoidRootPart.CFrame * CFrame.new(0, 12, 5))
                    end
                end
                lastCombatTime = tick()
                if tick() - lastCombatTime >= 30 and not CheckInComBat() then
                    FindNewEnemy()
                    lastCombatTime = tick()
                end
            end
