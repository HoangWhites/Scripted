...
GetBladeHits = function(SizeValue)
    ...
    for _, v in pairs(enemy) do
        if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 and v.Parent and getdistance(v.HumanoidRootPart.Position) < SizeValue + 5 then
            table.insert(Blade, v.HumanoidRootPart)
        end
    end
    return Blade
end
...
AddFast = function()
    ...
    if ac and ac.equiped and not plr.LocalPlayer.Character.Stun.Value ~= 0 then
        local blade = {}
    ...
    while wait(0.001) do
        if Scibidi["Fast Attack"] then
            repeat task.wait(Delay)
                AddFast()
            until Scibidi["Fast Attack"] == false or not plr.LocalPlayer.Character.Stun.Value ~= 0
        end
    end
end)
