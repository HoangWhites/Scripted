        ...
        if plr.Character.Humanoid.Health > 0 and plr.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if block and block.Parent == workspace then
                if Distance <= 250 then
                    ...
                    if Scibidi["Bypass Teleport"] and (targetcframe.Position - plh.Position).Magnitude >= 3125 and SpecialItem() then
                        wait(0.4)
                        tween:Cancel()
                        BypassTeleport(targetcframe)
                    end
                    if plr.Character.Humanoid.Sit and not Scibidi["Start Sea Event"] then
...
GetOwnerShip = function()
    for r, v in next, game.Workspace.Boats:GetChildren() do
        if v:FindFirstChild("Owner") and v.Name == Scibidi["Select Boats"] then
...
TweenObject = function(Object, Pos, Speed)
    ...
    if not Object:FindFirstChild("PartTween") then
        ...
        ObjectTween.CFrame = Object.CFrame
    ...
    tween1 = tweenService:Create(Object.PartTween, info, {CFrame = Pos})
    tween1:Play()
    local objectt = {}
    function objectt:Stop()
        tween:Cancel()
    end
    return objectt
end
...
function BringMob(vv)
    for r, v in next, workspace.Enemies:GetChildren() do
        if v.Name == vv.Name and v.Parent and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") then
            if not table.find(TableSeaMob, v.Name) then
        ...
        repeat task.wait()
            ...
            if dodge then
                if CheckAnimation(v.Humanoid) then
                    ...
                    Tween(v.HumanoidRootPart.CFrame * CFrame.new(0, 15, 9))
                end
            else
                if mobname.Name == "Cake Prince" then
                    if CheckAnimation(v.Humanoid) or game:GetService("Workspace")["_WorldOrigin"]:FindFirstChild("Ring") or game:GetService("Workspace")["_WorldOrigin"]:FindFirstChild("Fist") then
                        ...
                        Tween(v.HumanoidRootPart.CFrame * CFrame.new(-12, 15, 50))
                    end
                else
                    Tween(v.HumanoidRootPart.CFrame * CFrame.new(0, 15, 9))
                end
...
spawn(function()
    while wait() do
        if Scibidi["Auto Stats"] and Scibidi["Select Stats"] ~= "" then
...
local ToggleFarm = Tab2:AddToggle({
    Name = "Auto Farm",
...
ToggleFarm:AddToggle({
    Name = "Claim Quests(Bones & Cake)",
    Default = Scibidi["Claim Quests(Bones & Cake)"],
    Callback = function(Value)
        Save("Claim Quests(Bones & Cake)", Value)
    end
})
...
function AutoFarm()
    local QuestCake, Number, Mob, NameQuest  = CFrame.new(-2021.32007, 37.7982254, -12028.7295), 1, "Cookie Crafter", "CakeQuest1"
    local QuestBone, Number1, Mob1, NameQuest1 = CFrame.new(-9516.99316, 172.017181, 6078.46533), 2, "Posessed Mummy", "HauntedQuest2"
    if CheckStack("Method Farm") then
            ...
            if plr.Data.Level.Value >= 1 and plr.Data.Level.Value <= 119 then
                ...
                if plr.PlayerGui.Main.Quest.Visible then
                    ...
                    if CheckMob(MobLevelFarm) then
                        ...
                        TweenMobSpawn(MobData(), function() return not Scibidi["Auto Farm"] or CheckStack("Method Farm") end)
                    end
                else
                    ClaimQuestLevelFarm()
                end
        ...
        elseif Scibidi["Method Farm"] == "Cake Prince Farm" and Scibidi["Auto Farm"] and CheckSea(3) then
            if CheckMob("Cake Prince", true) then
                ...
                if Scibidi["Claim Quests(Bones & Cake)"] and not plr.PlayerGui.Main.Quest.Visible or Scibidi["Claim Quests(Bones & Cake)"] and not GetTextQuest(Mob) then
                    if (QuestCake.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 2 then
                        FireRemotes(2, "StartQuest", NameQuest, Number)
                    else
                        Tween(QuestCake)
                    end
                elseif not Scibidi["Claim Quests(Bones & Cake)"] or Scibidi["Claim Quests(Bones & Cake)"] and plr.PlayerGui.Main.Quest.Visible or Scibidi["Claim Quests(Bones & Cake)"] and GetTextQuest(Mob) then
        ...
        elseif Scibidi["Method Farm"] == "Bones Farm" and Scibidi["Auto Farm"] and CheckSea(3) then
            if Scibidi["Claim Quests(Bones & Cake)"] and not plr.PlayerGui.Main.Quest.Visible or Scibidi["Claim Quests(Bones & Cake)"] and not GetTextQuest(Mob1) then
                if (QuestBone.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 then
                    FireRemotes(2, "StartQuest", NameQuest1, Number1)
                else
                    Tween(QuestBone)
                end
            elseif not Scibidi["Claim Quests(Bones & Cake)"] or Scibidi["Claim Quests(Bones & Cake)"] and plr.PlayerGui.Main.Quest.Visible or Scibidi["Claim Quests(Bones & Cake)"] and GetTextQuest(Mob1) then
...
function AutoDoughKing()
    if not CheckMob("Dough King", true) then
        ...
        if getdistance(v.HumanoidRootPart.Position) >= 1500 then
            firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, workspace.Map.CakeLoaf.BigMirror.Main, 0)
            wait(0.1)
            firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, workspace.Map.CakeLoaf.BigMirror.Main, 1)
        else
    ...
    if CheckStack("Method Farm") then
        ...
        if CheckMobBuso() then
            ...
            Library:AddNotify({
                ...
                Content = "Go to Near Mob Spawn Please!!!",
                Time = 5
            })
            wait(5)
        end
    ...
    if CheckStack("Elite") then
        ...
        if CheckElite() then
            ...
            elseif plr.PlayerGui.Main.Quest.Visible == false or not GetTextQuest("Urban") and not GetTextQuest("Deandre") and not GetTextQuest("Diablo") then
                if (CFrame.new(-5418.892578125, 313.74130249023, -2826.2260742188).Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 then
...
spawn(function()
    while wait() do
        if CheckNotification("1000") and not plr.PlayerGui.Main.Timer.Visible then
            RaidDone = true
        end
    end
end)
...
getgenv().Nigga = false
function AutoRaid()
    if not CheckNotification("1000") and RaidDone then
        RaidDone = false
    end
    if not plr.PlayerGui.Main.Timer.Visible and not RaidDone then
        if not CheckPlayerBackpack("Special Microchip") and getgenv().Nigga == false and not RaidDone then
            BuyChips()
            IsRaid = false
            getgenv().Nigga = true
...
NpcGame = {}
function UpdateNpcFolder()
    for r, v in pairs(game:GetService("Workspace").NPCs:GetChildren()) do
        if not table.find(NpcGame, v) then
            table.insert(NpcGame, v)
        end
    end
end
UpdateNpcFolder()
        ...
        if Scibidi["Auto Kill Players After Trial"] and CheckSea(3) then
            pcall(function()
                for r, v in next, game.Workspace.Characters:GetChildren() do
                    if plr.PlayerGui.Main.Timer.Visible == true and v.Parent and v.Humanoid.Health > 0 and getdistance(v.HumanoidRootPart.Position) <= 350 and (v.HumanoidRootPart.Position - CFrame.new(8722.4707, 14887.5361, -95.0011368)) <= 350 then
                        repeat wait()
                            EquipWeapon()
                            EBuso()
...
Tab7:AddSlider({
    Name = "WalkSpeed",
    Max = 350,
    Min = tonumber(plr.Character.Humanoid.WalkSpeed),
...
Tab7:AddToggle({
    ...
    Default = Scibidi["Enable WalkSpeed"],
    Callback = function(Value)
        Save("Enable WalkSpeed", Value)
    end
})

game:GetService("RunService").RenderStepped:Connect(function()
    if plr.Character and plr.Character:FindFirstChild("Humanoid") and Scibidi["Enable WalkSpeed"] then
        plr.Character.Humanoid.WalkSpeed = Scibidi["WalkSpeed"]
    end
end)

Tab7:AddToggle({
    Name = "Walk On Water",
    Default = Scibidi["Walk On Water"],
    Callback = function(Value)
        Save("Walk On Water", Value)
        if Scibidi["Walk On Water"] then
...
Tab6:AddSlider({
    ...
    Max = 350,
    Min = 100,
    Default = Scibidi["Ship Speed"],
    Callback = function(Value)
        Save("Ship Speed", Value)
    end
})

Tab6:AddToggle({
    Name = "Enable Ship Speed",
    Default = Scibidi["Enable Ship Speed"],
    Callback = function(Value)
        Save("Enable Ship Speed", Value)
    end
})

spawn(function()
    while wait() do
        if Scibidi["Enable Ship Speed"] then
            if GetOwnerShip() then
                GetOwnerShip().VehicleSeat.MaxSpeed = Scibidi["Ship Speed"] or 100
            end
        end
    end
end)

Tab6:AddSeperator("Farming")
...
Tab6:AddToggle({
    Name = "Start Sea Event",
    Default = Scibidi["Start Sea Event"],
    Callback = function(Value)
        Save("Start Sea Event", Value)
    end
})
...
Tab6:AddDropdown({
    Name = "Select Boats",
    Options = TableBoats,
    Default = Scibidi["Select Boats"] or "PirateBrigade",
    Callback = function(Value)
        Save("Select Boats", Value)
    end
})

Tab6:AddToggle({
    Name = "Auto Shark",
    Default = Scibidi["Auto Shark"],
    Callback = function(Value)
        Save("Auto Shark", Value)
    end
})

Tab6:AddToggle({
    Name = "Auto Fish",
    Default = Scibidi["Auto Fish"],
    Callback = function(Value)
        Save("Auto Fish", Value)
    end
})

Tab6:AddToggle({
    Name = "Auto Piranha",
    Default = Scibidi["Auto Piranha"],
    Callback = function(Value)
        Save("Auto Piranha", Value)
    end
})

Tab6:AddToggle({
    Name = "Auto Terror Shark",
    Default = Scibidi["Auto Terror Shark"],
    Callback = function(Value)
        Save("Auto Terror Shark", Value)
    end
})
...
Tab6:AddToggle({
    Name = "Auto SeaBeasts",
    Description = "No Work",
    Default = Scibidi["Auto SeaBeasts"],
    Callback = function(Value)
        Save("Auto SeaBeasts", Value)
    end
})

function AutoFish()
    if CheckMob("Fish Crew Member") then
        repeat wait()
            KillMob(CheckMob("Fish Crew Member"), false, function() return not CheckMob("Fish Crew Member") or Scibidi["Auto Fish"] == false or Scibidi["Start Sea Event"] == false end)
        until CheckMob("Fish Crew Member") or Scibidi["Auto Fish"] == false or Scibidi["Start Sea Event"] == false or CheckMob("Fish Crew Member"):FindFirstChild("Humanoid").Health <= 0 or not CheckMob("Fish Crew Member").Parent
    end
end

function AutoPiranha()
    if CheckMob("Piranha") then
        repeat wait()
            KillMob(CheckMob("Piranha"), false, function() return not CheckMob("Piranha") or Scibidi["Auto Piranha"] == false or Scibidi["Start Sea Event"] == false end)
        until CheckMob("Piranha") or Scibidi["Auto Piranha"] == false or Scibidi["Start Sea Event"] == false or CheckMob("Piranha"):FindFirstChild("Humanoid").Health <= 0 or not CheckMob("Piranha").Parent
    end
end

function AutoTerrorshark()
    if CheckMob("Terrorshark") then
        repeat wait()
            KillMob(CheckMob("Terrorshark"), false, function() return not CheckMob("Terrorshark") or Scibidi["Auto Terror Shark"] == false or Scibidi["Start Sea Event"] == false end)
        until CheckMob("Terrorshark") or Scibidi["Auto Terror Shark"] == false or Scibidi["Start Sea Event"] == false or CheckMob("Terrorshark"):FindFirstChild("Humanoid").Health <= 0 or not CheckMob("Terrorshark").Parent
    end
end
function AutoShark()
    if CheckMob("Shark") then
        repeat wait()
            KillMob(CheckMob("Shark"), false, function() return not CheckMob("Shark") or Scibidi["Auto Shark"] == false or Scibidi["Start Sea Event"] == false end)
        until CheckMob("Shark") or Scibidi["Auto Shark"] == false or Scibidi["Start Sea Event"] == false or CheckMob("Shark"):FindFirstChild("Humanoid").Health <= 0 or not CheckMob("Terrorshark").Parent
    end
end
spawn(function()
    while wait() do
        pcall(function()
            if Scibidi["Start Sea Event"] and CheckSea(3) then
                if CheckMob(TableSeaMob) then
                    if tween1 then tween1:Cancel() end
                    plr.Character.Humanoid.Sit = false
                else
                    if GetOwnerShip() and getdistance(GetOwnerShip().VehicleSeat.Position) <= 2125 then
                        for r, v in pairs(workspace.Boats:GetDescendants()) do
                        ...
                        if plr.Character.Humanoid.Sit == false then
                            if tween1 then tween1:Cancel() end
                            Tween(GetOwnerShip().VehicleSeat.CFrame * CFrame.new(0, 2, 0))
                        else
                            TweenObject(GetOwnerShip().VehicleSeat, CFrame.new(-42183.3125, 40.8264961, 9999999999999999999999999999.999999999))
                        end
                    else
                        if getdistance(CFrame.new(-16927.451171875, 9.0863618850708, 433.8642883300781).Position) <= 10 then
                            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyBoat", Scibidi["Select Boats"])
                        else
...
spawn(function()
    while wait() do
        pcall(function()
            if Scibidi["Start Sea Event"] then
                if Scibidi["Auto Terror Shark"] then
                    AutoTerrorshark()
                end
                if Scibidi["Auto Piranha"] then
                    AutoPiranha()
                end
                if Scibidi["Auto Fish"] then
                    AutoFish()
                end
                if Scibidi["Auto Shark"] then
                    AutoShark()
                end
            end
        end)
