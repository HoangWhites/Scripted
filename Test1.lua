...
spawn(function()
    while task.wait() do
        if Scibidi["Fast Attack"] then
            pcall(function()
                local tool = CombatFrameworkR.activeController
                if tool and tool.equipped then
...
function AutoFarm()
    ...
    local QuestBone, Number1, Mob1, NameQuest1 = CFrame.new(-9516.99316, 172.017181, 6078.46533), 2, "Posessed Mummy", "HauntedQuest1"
    if CheckStack("Method Farm") then
        ...
        elseif Scibidi["Method Farm"] == "Cake Prince Farm" and Scibidi["Auto Farm"] and CheckSea(3) then
            if CheckMob("Cake Prince", true) then
                ...
                if Scibidi["Claim Quests(Bones & Cake)"] and not plr.PlayerGui.Main.Quest.Visible or not GetTextQuest(Mob) then
                    if getdistance(QuestCake.Position) > 10 then
                        Tween(QuestCake)
                    else
                        FireRemotes(2, "StartQuest", NameQuest, Number)
                    end
                elseif not Scibidi["Claim Quests(Bones & Cake)"] or Scibidi["Claim Quests(Bones & Cake)"] and plr.PlayerGui.Main.Quest.Visible or GetTextQuest(Mob) then
        ...
        elseif Scibidi["Method Farm"] == "Bones Farm" and Scibidi["Auto Farm"] and CheckSea(3) then
            if Scibidi["Claim Quests(Bones & Cake)"] and not plr.PlayerGui.Main.Quest.Visible or not GetTextQuest(Mob1) then
                if getdistance(QuestCake.Position) > 10 then
                    Tween(QuestBone)
                else
                    FireRemotes(2, "StartQuest", NameQuest1, Number1)
                end
            elseif not Scibidi["Claim Quests(Bones & Cake)"] or Scibidi["Claim Quests(Bones & Cake)"] and plr.PlayerGui.Main.Quest.Visible or GetTextQuest(Mob1) then
...
function AutoRaid()
    ...
    if not plr.PlayerGui.Main.Timer.Visible and RaidDone then
