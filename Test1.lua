...
AllBladeHits = function(Value)
    ...
    for _, Hit in ipairs(workspace.Enemies:GetChildren()) do
        ...
        if Humanoid and Humanoid.Health > 0 then
            ...
            if RootPart and plr:DistanceFromCharacter(RootPart.Position) < Value then
    ...
    for _, Char in ipairs(workspace.Characters:GetChildren()) do
        if Char.Name ~= plr.Name then
            ...
            if Humanoid and Humanoid.RootPart and Humanoid.Health > 0 then
                if plr:DistanceFromCharacter(Humanoid.RootPart.Position) < Value then
...
Attack = function(bladehit)
    local ac = CombatFrameworkR.activeController
    if ac and ac.equipped then
        local bladehit = {}
        if not (ac and ac.blades and #bladehit > 0) then return end
        if #bladehit > 0 then
...
Danh = function()
    Attack(AllBladeHits(60))
    Attack(AllPlayerHits(55))
end
function CheckStun()
        ...
        if dodge then
            ...
            if mobname.Name == "Cake Prince" or mobname.Name == "Dough King" then
                if CheckAnimation(v.Humanoid) or game:GetService("Workspace")["_WorldOrigin"]:FindFirstChild("Ring") or game:GetService("Workspace")["_WorldOrigin"]:FindFirstChild("Fist") then
                    ...
                    ToTarget(v.HumanoidRootPart.CFrame * CFrame.new(-12, 30, 80))
                end
            else
                ToTarget(v.HumanoidRootPart.CFrame * CFrame.new(0, 30, 9))
            end
