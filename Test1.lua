...
function SpecialItem()
    if not CheckPlayerBackpack("Torch") and not CheckPlayerBackpack("God's Chalice") and not CheckPlayerBackpack("Sweet Chalice") and not CheckPlayerBackpack("Fist Of Darkness") and not CheckPlayerBackpack("Hallow Essence") and not CheckPlayerBackpack("Sweet Chalice") then
        return true
    end
    return false
end
...
GetBladeHits = function(SizeValue)
	...
	for i,v in pairs(Enemies) do
		...
		if Human and Human.RootPart and Human.Health > 0 and plr:DistanceFromCharacter(Human.RootPart.Position) < SizeValue + 5 then
			table.insert(Blade, Human.RootPart)
		end
	end
	return Blade
end
...
AddFast = function()
    ...
    if ac and ac.equipped and not plr.Character.Stun.Value ~= 0 then
        local blade = {}
        ...
        if Scibidi["Fast Attack"] then
            pcall(function()
                repeat task.wait(Delay)
                    task.spawn(function()
                        pcall(task.spawn, AddFast)
                    end)
                until Scibidi["Fast Attack"] == false or not plr.Character.Stun.Value ~= 0
            end)
        end
    end
end)
...
function GetFruitSpawn()
    for r, v in pairs(game.Workspace:GetChildren()) do
        if v:IsA("Model") and string.find(v.Name, "Fruit") and v:FindFirstChild("Handle") and v.Parent then
            return v
        end
    end
end
...
function CheckStack(NameFunc)
    if NameFunc == "Method Farm" then
        ...
        elseif Scibidi["Auto Electric Claw"] and CheckPlayerBackpack("Electro") and GetMeleeMastery("Electro", 400) and RS.Remotes.CommF_:InvokeServer("BuyElectricClaw", true) ~= 1 then return true
        ...
        elseif Scibidi["Auto Third Sea"] and CanGoThirdSea() and CheckSea(2) then return true
        elseif Scibidi["Auto Dough King"] and CheckPlayerBackpack("God's Chalice") or CheckPlayerBackpack("Sweet Chalice") then return true
        elseif Scibidi["Auto Rengoku"] and CheckPlayerBackpack("Hidden Key") then return true
        end
...
function TweenMobSpawn(value, loopstop)
    pcall(function()
        ...
        if typeof(value) == "table" then
            for r, v in next, value do
                MobPart = GetPartMob(v)
                repeat task.wait()
                if loopstop and loopstop() then return end
                    NoClip = true
                    Tween(MobPart.CFrame * CFrame.new(0, 20, 0))
                until (MobPart.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 20 or CheckMob(value) or (loopstop and loopstop())
            end
        else
            MobPart = GetPartMob(value)
            repeat task.wait()
                if loopstop and loopstop() then return end
                NoClip = true
                Tween(MobPart.CFrame * CFrame.new(0, 20, 0))
            until CheckMob(value) or (loopstop and loopstop())
        end
    end)
end
...
function GetSaber()
    if not GetSaberDoor() then
                ...
                if not CheckPlayerBackpack("Cup") then
                    if not GetDoorCup() then
                        if not CheckPlayerBackpack("Torch") then
                            ...
                            EquipTool("Torch")
                            Tween(CFrame.new(1114.3269, 5.2647109, 4347.73047, -0.669475853, -4.66796521e-08, 0.742833853, -2.86030915e-08, 1, 3.70615503e-08, -0.742833853, 3.56446872e-09, -0.669475853))
                        end
                    else
        ...
        until not Scibidi["Auto Saber"] or GetPlayerInventory("Saber") or not v.Parent or v.Humanoid.Health <= 0
    end
end
        ...
        if RS.Remotes.CommF_:InvokeServer("DressrosaQuestProgress", "Dressrosa") ~= 0 then
            if game.Workspace.Map.Ice.Door.CanCollide then
                if not CheckPlayerBackpack("Key") then
                    if (CFrame.new(4852.2895507813, 5.651451587677, 718.53070068359).Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 30 then
...
local AutoThirdSeaToggle = Tab3:AddToggle({
    Name = "Auto Third Sea",
...
AutoThirdSeaToggle:AddToggle({
    Name = "Hop Find Don Swan",
    Default = Scibidi["Hop Find Don Swan"],
    Callback = function(Value)
        Save("Hop Find Don Swan", Value)
    end
})
...
function ToSea3()
    if game.PlaceId == 4442272183 and plr.Data.Level.Value >= 1500 then
        if RS.Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 3 then
            if RS.Remotes.CommF_:InvokeServer("TalkTrevor", "1") ~= 0 then 
                if #checkfruitbackpack() >= 1 then
            ...
            elseif not RS.Remotes.CommF_:InvokeServer("ZQuestProgress", "Check") then
                ...
                if v then
                    repeat wait()
                        KillMob(v, false, function() return not Scibidi["Auto Third Sea"] end)
                    until not Scibidi["Auto Third Sea"] or v.Humanoid.Health <= 0 or not v.Parent
                else
                    if Scibidi["Hop Find Don Swan"] then
                        HopServer()
                    end
                end
            elseif RS.Remotes.CommF_:InvokeServer("ZQuestProgress", "Check") == 0 then
...
spawn(function()
    while task.wait() do
        pcall(function()
            if Scibidi["Auto Third Sea"] and plr.Data.Level.Value >= 1500 and CheckSea(2) then
                FireRemotes(2, "TravelZou")
                ToSea3()
            end
        end)
    ...
    if CheckStack("Elite") then return
        ...
        if CheckElite() then
            if plr.PlayerGui.Main.Quest.Visible and GetTextQuest("Urban") or GetTextQuest("Deandre") or GetTextQuest("Diablo") then
                ...
                repeat wait()
                    KillMob(v, false, function()
                        return not Scibidi["Auto Elite"] or CheckStack("Elite") or plr.PlayerGui.Main.Quest.Visible == false or not GetTextQuest("Urban") and not GetTextQuest("Deandre") and not GetTextQuest("Diablo")
                    end)
...
spawn(function()
    while wait() do
        if Scibidi["Auto Yama"] and CheckSea(3) and not GetPlayerInventory("Yama") then
            GetYama()
        end
    end
end)
    ...
    while wait() do
        if Scibidi["Auto Store DF"] and not stopstore then
            ...
            for r, v in next, plr.PlayerGui.Notifications:GetChildren() do
                if string.find(v.Text, "only") or string.find(v.Text, "Can't") or string.find(v.Text, "Only") then
                    v.Visible = false
                end
            end
        end
...
spawn(function()
    while wait() do
        if CheckNotification("1000") and IsRaid then
            getgenv().Nigga = true
        end
    end
end)
...
CanbuyElectricClaw = false
function AutoElectricClaw()
    if RS.Remotes.CommF_:InvokeServer("BuyElectricClaw", true) == 1 then
        StartGetElectricClaw = false
        return
    end
    if RS.Remotes.CommF_:InvokeServer("BuyElectricClaw", true) ~= 1 and not CanbuyElectricClaw then
        if CheckSea(3) then
            if CheckPlayerBackpack("Electro") then
                if GetMeleeMastery("Electro", 400) then
                    repeat wait()
                        StartGetElectricClaw = true
                        RS.Remotes.CommF_:InvokeServer("BuyElectricClaw","Start")
                        wait(1)
                        RS.Remotes.CommF_:InvokeServer("requestEntrance",Vector3.new(-12549.7236, 337.511505, -7470.36328))
                        wait(1)
                        CanbuyElectricClaw = true
                        if CanbuyElectricClaw then
                            if plr.Data.Beli.Value < 3000000 then
                                Library:AddNotify({
                                    Title = "Script Warning",
                                    Content = "Farming Beli to 3M Beli Please",
                                    Time = 5
                                })
                                wait(5)
                            elseif plr.Data.Fragments.Value < 5000 then
                                TaoDangDiRaid = true
                                AutoRaid()
                            elseif plr.Data.Fragments.Value >= 5000 and plr.Data.Beli.Value >= 3000000 then
                                FireRemotes(2, "BuyElectricClaw")
                            end
                        end
                    until RS.Remotes.CommF_:InvokeServer("BuyElectricClaw", true) == 1 or not Scibidi["Auto Electric Claw"]
                end
            else
                FireRemotes(2, "BuyElectro")
            end
...
spawn(function()
    while task.wait() do
        if Scibidi["Auto Electric Claw"] and RS.Remotes.CommF_:InvokeServer("BuyElectricClaw", true) ~= 1 then
            pcall(function()
                FireRemotes(2, "BuyElectricClaw")
                AutoElectricClaw()
            end)
...
spawn(function()
    while wait() do
        if Scibidi["Walk On Water"] then
            game.Workspace.Map:FindFirstChild("WaterBase-Plane").Size = Vector3.new(1000, 113, 1000)
        else
            game.Workspace.Map:FindFirstChild("WaterBase-Plane").Size = Vector3.new(1000, 80, 1000)
        end
    end
end)
----------------------------------------------------------------------------------------------------------------------------------------------------------------
Library:BuildUI()
print("[Build GUI]")
for r, v in next, plr:GetDescendants() do
