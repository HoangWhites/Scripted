...
function ToTarget(TargetCFrame)
    ...
    if plr.Character:FindFirstChild("Humanoid") and plr.Character.Humanoid.Health > 0 and plr.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if Aaa <= 200 then
            ...
            if config["Bypass Teleport"] and (plr.Character:WaitForChild("PrimaryPart").Position - TargetCFrame.Position).Magnitude >= 3125 then
                if plr.Character.Humanoid.Health > 0 then
                    wait(.4)
                    if tween then tween:Pause() tween:Cancel() end
...
spawn(function()
    while wait() do
        if Velocity then
            if not plr.Character:WaitForChild("Head"):FindFirstChild("Velocity") then
                local Velocity = Instance.new("BodyVelocity", plr.Character.Head)
            ...
            if plr.Character:WaitForChild("Head"):FindFirstChild("Velocity") then
                plr.Character:WaitForChild("Head"):FindFirstChild("Velocity"):Destroy()
            end
        end
    end
end)
...
function CheckStringQuests(ac)
    return string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, ac)
end
    ...
    if CheckingStack("Normal Farm") then
        ...
        elseif config["Method Farm"] == "Cake Prince Farm" then
            if DetectEnemy("Cake Prince", true) then
                ...
                if config["Claim Quest"] and not CheckStringQuests("Cookie Crafter") or config["Claim Quest"] and not plr.PlayerGui.Main.Quest.Visible == false then
                ...
                elseif not config["Claim Quest"] or CheckStringQuests("Cookie Crafter") and config["Claim Quest"] or config["Claim Quest"] and plr.PlayerGui.Main.Quest.Visible == true then
        ...
        elseif config["Method Farm"] == "Bones Farm" then
            if config["Claim Quest"] and not CheckStringQuests("Posessed Mummy") or config["Claim Quest"] and not plr.PlayerGui.Main.Quest.Visible == false then
            ...
            elseif not config["Claim Quest"] or CheckStringQuests("Posessed Mummy") and config["Claim Quest"] or config["Claim Quest"] and plr.PlayerGui.Main.Quest.Visible == true then
