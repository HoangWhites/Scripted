...
function ToTarget(TargetCFrame)
    ...
    if plr.Character:FindFirstChild("Humanoid") and plr.Character.Humanoid.Health > 0 and plr.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if Aaa <= 200 then
            ...
            if config["Bypass Teleport"] and (plr.Character:WaitForChild("HumanoidRootPart").Position - TargetCFrame.Position).Magnitude >= 3125 and plr.Character:WaitForChild("Humanoid").Health > 0 then
                if plr.Character.Humanoid.Health > 0 then
                    wait(.4)
                    if tween then tween:Pause() tween:Cancel() end
    ...
    if CheckingStack("Normal Farm") then
        ...
        elseif config["Method Farm"] == "Cake Prince Farm" then
            if DetectEnemy("Cake Prince", true) then
                ...
                if config["Claim Quest"] and not CheckStringQuests("Cookie Crafter") or config["Claim Quest"] and plr.PlayerGui.Main.Quest.Visible == false then
