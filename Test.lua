...
AllBladeHits = function(Value)
    ...
    for _, Hit in ipairs(Workspace.Enemies:GetChildren()) do
        ...
        if Humanoid and Humanoid.Health > 0 then
            ...
            if RootPart and plr.LocalPlayer:DistanceFromCharacter(RootPart.Position) < Value then
...
AllPlayerHits = function(Value)
    ...
    for _, Char in ipairs(Workspace.Characters:GetChildren()) do
        if Char.Name ~= plr.LocalPlayer.Name then
            local Humanoid = Char and Char:FindFirstChildOfClass("Humanoid")
            if Humanoid and Humanoid.RootPart and Humanoid.Health > 0 then
                if plr.LocalPlayer:DistanceFromCharacter(Humanoid.RootPart.Position) < Value then
